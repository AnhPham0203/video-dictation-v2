...
const Index = () => {
  ...
  const fetchCaptions = async (videoId: string) => {
    ...
    try {
      // Fetch from the serverless function (local or remote based on env config)

      const response = await fetch(
        buildApiUrl(`/api/get-captions?videoId=${videoId}`),
        { cache: "no-store" }
      );
      ...
      if (data.sentences && data.sentences.length > 0) {
        ...
        const processedSentences = data.sentences.map(
          (sentence: ApiSentence, index: number) => {
            ...
            const duration = Number(sentence.duration) || 0;
